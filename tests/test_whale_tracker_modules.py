...
def test_orderbook_module_bid_heavy_signals_long():
    module = OrderbookImbalanceModule(up_pressure_ratio=2.5, down_pressure_ratio=0.4)
    signal = module.evaluate(bid_volume=300, ask_volume=100)  # oran 3.0
    assert signal.triggered is True
    assert signal.direction == "LONG"


def test_orderbook_module_ask_heavy_signals_short():
    module = OrderbookImbalanceModule(up_pressure_ratio=2.5, down_pressure_ratio=0.4)
    signal = module.evaluate(bid_volume=30, ask_volume=100)  # oran 0.3
    assert signal.triggered is True
    assert signal.direction == "SHORT"
...
def test_trap_scorer_picks_the_heavier_weighted_direction():
    ...
    signals = {
        ...
        "orderbook": OrderbookImbalanceModule().evaluate(bid_volume=30, ask_volume=100),  # SHORT, ağırlık .10 -> katkı 10
    }
